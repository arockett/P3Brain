...
public:
    ...
    void archive(vector<Organism*> population, int flush = 0) {
        ...
        if (outputMethod == 0) {
            ...
            if ((Global::update % pruneInterval == 0) || (flush == 1)) {
                ...
                Organism* sampleOrg = population[0];
                Organism* MRCA = sampleOrg->getMostRecentCommonAncestor();

                if (files.find("data.csv") == files.end()) { // if file has not be initialized yet
                    files[DataFileName] = sampleOrg->dataMap.getKeys();
                }

                vector<Organism*> LOD = sampleOrg->getLOD();       // get line of decent
                ...
                while ((effective_MRCA->timeOfBirth >= nextDataWrite) && (nextDataWrite <= Global::updates)) { // if there is convergence before the next data interval
                    Organism* current = LOD[nextDataWrite - lastPrune];
                ...
                while ((effective_MRCA->timeOfBirth >= nextGenomeWrite) && (nextGenomeWrite <= Global::updates)) { // if there is convergence before the next data interval
                    Organism* current = LOD[nextGenomeWrite - lastPrune];
                    ...
                    while (!curr_checkPoint.empty()) {
                        ...
                        for (auto org : curr_checkPoint) {
                            ...
                            if ((orgTrackingCounts[org->ID] == org->referenceCounter)) { // if only the tracker is looking at this organism it can be deleted
                                ...
                                set<Organism*> parents = org->parents;
                                delete (org); // the org has no decendants and no tracker is looking at it. It's time to delete the organism.
                                for (auto parent : parents) {
                                    //if (parent is in this checkpoint, but not in current then it's been tested and needs to be retested)
                                    //add this parent to the retest set - keep running until retest is empty.
                                    if (checkPointTracker[trackedCheckPoints[i]].find(parent) != checkPointTracker[trackedCheckPoints[i]].end()) { // if this parent is in the current checkpoint
                                        if (checkedOrgs.find(parent) != checkedOrgs.end()) { // and they have already been evaluated
                                            recheckCheckPoint.insert(parent);
                                        }
                                    }
                                }
                            }
