...
bool& Gate::usingFBGate = Parameters::register_parameter("fbGate", false,
        "set to true to enable feedback gates", "GATE TYPES");
bool& Gate::usingGPGate = Parameters::register_parameter("gpGate", false,
        "set to true to enable GP (what?) gates", "GATE TYPES");
bool& Gate::usingThGate = Parameters::register_parameter("thGate", false,
        "set to true to enable theta gates", "GATE TYPES");
bool& Gate::usingEpsiGate = Parameters::register_parameter("epsiGate", false,
        "set to true to enable epsilon gates", "GATE TYPES");
bool& Gate::usingVoidGate = Parameters::register_parameter("voidGate", false,
        "set to true to enable void gates", "GATE TYPES");
...
void Gate::setupGates() {
    ...
    if (usingDetGate) {
        AddGate(43,
                [](Genome* genome,int pos) {return make_shared<DeterministicGate>(genome,pos);});
        Global::inUseGateTypes.insert(43);
    }
    if (usingFBGate) {
        AddGate(44,
                [](Genome* genome,int pos) {return make_shared<FeedbackGate>(genome,pos);});
        Global::inUseGateTypes.insert(44);
    }
    if (usingGPGate) {
        AddGate(45,
                [](Genome* genome,int pos) {return make_shared<GPGate>(genome,pos);});
        Global::inUseGateTypes.insert(45);
    }
    if (usingThGate) {
        AddGate(46,
                [](Genome* genome,int pos) {return make_shared<Thresholdgate>(genome,pos);});
        Global::inUseGateTypes.insert(46);
    }
    if (usingEpsiGate) {
        AddGate(47,
                [](Genome* genome,int pos) {return make_shared<FixedEpsilonGate>(genome,pos);});
        Global::inUseGateTypes.insert(47);
    }
    if (usingVoidGate) {
        AddGate(48,
                [](Genome* genome,int pos) {return make_shared<VoidGate>(genome,pos);});
        Global::inUseGateTypes.insert(47);
    }
...
void Gate::getInputsAndOutputs(const vector<int> insRange,
        vector<int> outsRange, int& genomeIndex, Genome* genome) { // (max#in, max#out,currentIndexInGenome,genome,codingRegions)
    ...
    if (insRange[1] > 0) {
        getSomeBrainAddresses(numInputs, insRange[1], inputs, genomeIndex,
                genome, 11);
    }
    if (outsRange[1] > 0) {
