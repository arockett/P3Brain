 ...
 *
 * uses Data::inUseGateTypes to determine which values may code for gates.
 *
...
void MutationAnalyse::evalPossibleAgents(Genome* genome, int brainStates, World *world, double &avgWildTypeFitness,
	...
	for (auto position : possStartCodons[1]) {
		...
		if (agentCodingReg.find(testSite) != agentCodingReg.end()) { //if the site before this site is in the list of coding regions
	...
	//avgFitnessChange=avgFitnessChange/agentCodingReg.size();
	//cout << "Agent ID =" << popID << " Coding Region Size = " << agentCodingReg.size() << "  Genome Size = " << testGenome->sites.size() <<"  Average Total Fitness Change =" << avgFitnessChange << "\n";
...
void MutationAnalyse::test_point_mutations_and_output(int gen, Genome* tester, bool avePerVar, bool allVar,
	...
	Agent* subAgent = new Agent((Genome*) subject, nrOfBrainStates); //make the corresponding agent
...
/*
 ...
 for(int agentID=0;agentID<popScores.size();agentID++){
 cout << "Agent ID =" << agentID << "\n";
 for(int siteIndex=0;siteIndex<popScores[agentID].size();siteIndex++){ //for every int key of the map
