...
FeedbackGate::FeedbackGate(shared_ptr<Genome> genome, int genomeIndex) {
  ...
  posFBNode = genome->extractValue(genomeIndex, { 0, 255 }, Genome::DATA_CODE);
  negFBNode = genome->extractValue(genomeIndex, { 0, 255 }, Genome::DATA_CODE);
  nrPos = genome->extractValue(genomeIndex, { 0, 3 }, Genome::DATA_CODE);
  nrNeg = genome->extractValue(genomeIndex, { 0, 3 }, Genome::DATA_CODE);
  ...
  for (i = 0; i < _yDim; i++) {
    inputs[i] = genome->extractValue(genomeIndex, { 0, 255 }, Genome::IN_ADDRESS_CODE);
  }
  for (; i < 4; i++) {
    genome->extractValue(genomeIndex, { 0, 255 });
  }
  for (i = 0; i < _xDim; i++) {
    outputs[i] = genome->extractValue(genomeIndex, { 0, 255 }, Genome::OUT_ADDRESS_CODE);
  }
  for (; i < 4; i++) {
    genome->extractValue(genomeIndex, { 0, 255 });
  }
  ...
  for (i = 0; i < nrPos; i++) {
    posLevelOfFB[i] = ((double) (1 + genome->extractValue(genomeIndex, { 0, 255 }, Genome::DATA_CODE))) / 256.0;
  }
  for (; i < 4; i++) {
    genome->extractValue(genomeIndex, { 0, 255 });
  }
  for (i = 0; i < nrNeg; i++) {
    negLevelOfFB[i] = ((double) (1 + genome->extractValue(genomeIndex, { 0, 255 }, Genome::DATA_CODE))) / 256.0;
  }
  for (; i < 4; i++) {
    genome->extractValue(genomeIndex, { 0, 255 });
  }
  ...
  for (i = 0; i < (1 << _yDim); i++) {
    ...
    double S = 0.0;
    for (j = 0; j < (1 << _xDim); j++) {
      table[i][j] = (double) genome->extractValue(genomeIndex, { 0, 255 }, Genome::DATA_CODE);
      S += table[i][j];
    }
    //normalize the row
    if (S == 0.0) {
