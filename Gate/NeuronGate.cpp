...
const int NeuronGate::defaultDischargeBehavior = Parameters::register_parameter("dischargeBehavior", 0, "what happens when gate fires (-1: let genome decide, 0: clear chrage, 1: 'reduce' bu delivery charge, 2: 1/2 charge)", "GATES - NEURON");
...
//int numInputs;
//int dischargeBehavior; //-1 or 0,1,2;
//double thresholdRange;
//bool allowRepression; // if false, neuronGates fire if they exceed charge, if true, gates can exist which always fire, unless they exceed charge
//double decayRate;
//double deliveryChargeRange; //
//double deliveryError = .01;
