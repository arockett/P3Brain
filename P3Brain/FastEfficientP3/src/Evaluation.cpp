...
// Return fitness of solution for MatchMaker problem
float MatchMaker::evaluate( const vector<bool>& solution )
{
    // Initialize record keeping variables
    int sum = 0;
    int max = solution.size() / group_size;

    // For each group (including the last partial group if it exists)
    for ( int n = 0; n <= max; n++ )
    {
        bool same = true;
        int i = n * group_size;
    ...
    // Calculate the fitness based on total possible groups
    if ( solution.size() % group_size != 0 )
        max++;
...
float NearestNeighborNK::solve(vector<bool>& solution, bool maximize) {
    ...
    for (size_t left = 0; left < numbers; left++) {
      for (size_t right = 0; right < numbers; right++) {
        ...
        for (size_t middle = 0; middle < numbers; middle++) {
          ...
          if ((maximize && utility[left][right] < current) || ( !maximize && utility[left][right] > current)) {
  ...
  for (size_t left = 0; left < numbers; left++) {
    for (size_t right = 0; right < numbers; right++) {
      ...
      if ((maximize && fitness < current) || ( !maximize && fitness > current)) {
        fitness = current;
        best_left = left;
        best_right = right;
      }
