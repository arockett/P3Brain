...
double& BerryWorld::rewardForFood1 = Parameters::register_parameter(
        "BERRY_rewardForFood1", 1.0, "reward for eating a Food1",
        "WORLD - BERRY");
double& BerryWorld::rewardForFood2 = Parameters::register_parameter(
        "BERRY_rewardForFood2", 1.0, "reward for eating a Food2",
        "WORLD - BERRY");
double& BerryWorld::rewardForFood3 = Parameters::register_parameter(
        "BERRY_rewardForFood3", 1.0, "reward for eating a Food3",
        "WORLD - BERRY");
double& BerryWorld::rewardForFood4 = Parameters::register_parameter(
        "BERRY_rewardForFood4", 1.0, "reward for eating a Food4",
        "WORLD - BERRY");
double& BerryWorld::rewardForFood5 = Parameters::register_parameter(
        "BERRY_rewardForFood5", 1.0, "reward for eating a Food5",
        "WORLD - BERRY");
double& BerryWorld::rewardForFood6 = Parameters::register_parameter(
        "BERRY_rewardForFood6", 1.0, "reward for eating a Food6",
        "WORLD - BERRY");
double& BerryWorld::rewardForFood7 = Parameters::register_parameter(
        "BERRY_rewardForFood7", 1.0, "reward for eating a Food7",
        "WORLD - BERRY");
double& BerryWorld::rewardForFood8 = Parameters::register_parameter(
        "BERRY_rewardForFood8", 1.0, "reward for eating a Food8",
        "WORLD - BERRY");

int& BerryWorld::xDim = Parameters::register_parameter("BERRY_WorldX", 8,
        "world X size", "WORLD - BERRY");
int& BerryWorld::yDim = Parameters::register_parameter("BERRY_WorldY", 8,
        "world Y size", "WORLD - BERRY");
bool& BerryWorld::borderWalls = Parameters::register_parameter(
        "BERRY_makeBorderWalls", true,
...
bool& BerryWorld::clearOutputs = Parameters::register_parameter(
        ...
        "if true outputs will be cleared on each world update",
        "WORLD - BERRY");
...
bool& BerryWorld::allowMoveAndEat = Parameters::register_parameter(
        ...
        "WORLD - BERRY");
bool& BerryWorld::senseDown = Parameters::register_parameter("BERRY_senseDown",
        true, "if true, Agent can sense what it's standing on",
        "WORLD - BERRY");
bool& BerryWorld::senseFront = Parameters::register_parameter(
        "BERRY_senseFront", true,
...
double BerryWorld::testIndividual(Organism *org, bool analyse) {
    int grid[xDim][yDim];
    int i, j;
    int xp = xDim / 2, yp = yDim / 2, dir = Random::getInt(7); //xp and yp are x and y positions. brain starts at center of grid and with a random rotation
    ...
    for (int t = 0; t < worldUpdates; t++) { //run brain for "worldUpdates" brain updates

        here = grid[xp][yp]; // where the brain is standing
        front = grid[loopMod((xp + xm[dir]), (xDim))][loopMod((yp + ym[dir]),
                (yDim))];
        leftFront = grid[loopMod((xp + xm[(dir - 1) & 7]), (xDim))][loopMod(
                (yp + ym[(dir - 1) & 7]), (yDim))];
        rightFront = grid[loopMod((xp + xm[(dir + 1) & 7]), (xDim))][loopMod(
                (yp + ym[(dir + 1) & 7]), (yDim))];
        ...
        if ((output2 == 1) && (grid[xp][yp] != EMPTY)) { // eat food here (if there is food here)
            ...
            org->dataMap.Append("foodList", grid[xp][yp]);
            lastFood = grid[xp][yp];
        ...
        if ((output2 == 0) || (allowMoveAndEat == 1)) { // if we did not eat or we allow moving and eating in the same world update
            ...
            case 2: //turn right
                ...
                break;
            case 3: //move forward
                if (grid[loopMod((xp + xm[dir]), (xDim))][loopMod(
                        (yp + ym[dir]), (yDim))] != WALL) { // if the proposed move is not a wall
